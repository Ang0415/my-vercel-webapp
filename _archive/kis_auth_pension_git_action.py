# kis_auth_pension_git_action.py
# KIS 연금저축 인증 모듈 (GitHub Actions용)
...
def getEnv():
    ...
    kis_cano_pension = os.environ.get('KIS_CANO_PENSION')
    
    if kis_appkey and kis_secret and kis_cano_pension:
        env_input = os.environ.get('KIS_ENV', 'prod').lower()
        print(f"✅ [GitAction] KIS Pension 설정 로드 완료 (환경변수, Mode={env_input})")
        ...
        return {
            'my_app': kis_appkey,
            'my_sec': kis_secret,
            'my_acct': kis_cano_pension, 
            'my_acct_pension': kis_cano_pension, 
            'my_prod': '22',
            'my_token': '',
...
def save_token_to_file(token, expire_time_str):
    # GitHub Action에서는 파일 저장이 굳이 필요 없지만, 
    # 로컬 테스트 및 흐름 호환성을 위해 유지 (저장 실패해도 무시)
    try:
...
def auth(svr="prod", product="22"):
    ...
    # 1. 파일 토큰 확인 (로컬 실행 시 유용, GitHub Action은 보통 매번 새로 받음)
    ...
    try:
        ...
        if new_token and expire_time:
            ...
            print("✅ [GitAction] KIS Pension 토큰 발급 성공")
            return True
        return False
    except requests.exceptions.HTTPError as e:
        print(f"❌ [GitAction] KIS Pension 인증 실패 (HTTP {e.response.status_code}): {e}")
        if e.response is not None:
             print(f"   [상세 에러] {e.response.text}")
        return False
    except Exception as e:
        print(f"❌ [GitAction] KIS Pension 인증 실패: {e}")
        return False
