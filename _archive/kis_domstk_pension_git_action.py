# -*- coding: utf-8 -*-
"""
예제 기반 kis_domstk.py 전체 코드 (GitHub Action용)
"""
...
try:
    import kis_auth_pension_git_action as kis
except ModuleNotFoundError:
    try:
        import kis_auth_pension as kis
    except ModuleNotFoundError:
        print("❌ 인증 모듈(kis_auth_pension)을 찾을 수 없습니다.")
        raise
...
def get_inquire_balance_obj():
    ...
    try:
        ...
        print("\n📊 [STEP 2] 주식잔고조회 (잔고현황)")
        print(f"💰 평가금액: {int(output2['tot_evlu_amt']):,}원")
        # print(f"💵 입금금액: {int(output2['pchs_amt_smtl_amt']):,}원") # 생략 가능
        # print(f"📉 평가손익: {int(output2['evlu_pfls_smtl_amt']):,}원") # 생략 가능
    except Exception as e:
        pass # print("❗ 잔고 요약 정보가 없습니다.", e)
